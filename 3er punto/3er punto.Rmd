...
distancia <- c(50, 100, 150, 200, 250, 300, 400)


velocidad <- distancia / tiempos.seg

modelo_nls <- nls(velocidad ~ a * tiempos.seg[, 1] + b, 
                   data = data.frame(velocidad, tiempos.seg),
                   start = list(a = 1, b = 0))
summary(modelo_nls)
```
...
# Supongamos que los datos de distancia y tiempo están en un dataframe llamado 'datos'
# Debes tener una columna llamada 'distancia' y una columna llamada 'tiempo'

# Ajusta un modelo de segmentos de línea recta
modelo_segmentado <- segmented(lm(tiempos.seg ~ distancia, data = temp), 
                                seg.Z = ~distancia, psi = list(distancia = c(20, 200, 370)))
